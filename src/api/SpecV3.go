...
func (spec SpecV3) parseMediaType(ymlMT imap) (specMT MediaType) {
	if ymlMT != nil {
		...
		// Found in the spec generated by the Stoplight OAS Studio
		if ymlMT["x-examples"] != nil {
			for ymlExampleName, ymlExample := range ymlMT["x-examples"].(imap) {
				specMT.Examples[ymlExampleName.(string)] = ExampleObject(ymlExample.(imap))
			}
		}
	}
...
func (spec SpecV3) parseSchema(name string, ymlSchemaM imap) *Schema {
	...
	specMT := spec.parseMediaType(ymlSchemaM)
